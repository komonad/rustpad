...
// Incoming external event from core
pub struct ExtEvent {
    event_sink: ExtEventSink,
}

impl ExtEvent {
    pub const VIEW_ID: Selector<bool> = Selector::new("code-editor.view-id");
    pub const APPLY_UPDATE: Selector<bool> = Selector::new("code-editor.apply-update");
    pub const SCROLL_TO: Selector<bool> = Selector::new("code-editor.scroll-to");
    pub const CORE: Selector<bool> = Selector::new("code-editor.core");
    pub const UNDO: Selector<bool> = Selector::new("code-editor.undo");
    pub const REDO: Selector<bool> = Selector::new("code-editor.redo");
    pub const UPPER_CASE: Selector<bool> = Selector::new("code-editor.upper-case");
    pub const LOWER_CASE: Selector<bool> = Selector::new("code-editor.lower-case");
    pub const TRANSPOSE: Selector<bool> = Selector::new("code-editor.transpose");
    pub const ADD_CURSOR_ABOVE: Selector<bool> = Selector::new("code-editor.add-cursor-above");
    pub const ADD_CURSOR_BELOW: Selector<bool> = Selector::new("code-editor.add-cursor-below");
    pub const SINGLE_SELECTION: Selector<bool> = Selector::new("code-editor.single-selection");
    pub const SELECT_ALL: Selector<bool> = Selector::new("code-editor.select-all");
}
...
impl<T: EditableText + TextStorage> CodeEditor<T> {
    /// Create a new TextBox widget.
    pub fn new() -> Self {
        let mut placeholder = TextLayout::from_text("");
        placeholder.set_text_color(theme::PLACEHOLDER_COLOR);
        let mut scroll = Scroll::new(Padding::new(theme::TEXTBOX_INSETS, TextComponent::default()))
    ...
    pub fn multiline() -> Self {
        let mut this = CodeEditor::new();
        this.inner.set_enabled_scrollbars(druid::scroll_component::ScrollbarsEnabled::Both);
...
impl<T> CodeEditor<T> {
    ...
    pub fn set_text_size(&mut self, size: impl Into<KeyOrValue<f64>>) {
        ...
        let size = size.into();
        self.text_mut().borrow_mut().layout.set_text_size(size.clone());
...
impl<T: TextStorage + EditableText> CodeEditor<T> {
...
impl<T: TextStorage + EditableText> Widget<T> for CodeEditor<T> {
        ...
        match event {
            ...
            Event::Paste(ref item) if self.text().can_write() => {
                if let Some(string) = item.get_string() {
                    let text =
                        if self.multiline { &string } else { string.lines().next().unwrap_or("") };
                    if !text.is_empty() {
    ...
    #[instrument(name = "CodeEditor", level = "trace", skip(self, ctx, old, data, env))]
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &T, env: &Env) -> Size {
        ...
        trace!("Computed layout: size={}, baseline_offset={:?}", size, baseline_off);
        size
    }
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &T, env: &Env) {
        ...
        let border_color =
            if is_focused { env.get(theme::PRIMARY_LIGHT) } else { env.get(theme::BORDER_DARK) };
