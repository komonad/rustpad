...
use druid::text::ImeHandlerRef;

use super::{
    EditableText, ImeInvalidation, InputHandler, Movement, Selection, TextAction,
...
impl<T: TextStorage + EditableText> Widget<T> for TextComponent<T> {
    ...
    fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, event: &LifeCycle, data: &T, env: &Env) {
        match event {
            ...
            LifeCycle::Internal(druid::InternalLifeCycle::ParentWindowOrigin)
                if self.can_write() =>
            {
                if self.can_write() {
...
impl<T: TextStorage + EditableText> EditSession<T> {
    ...
    fn do_action(&mut self, buffer: &mut T, action: TextAction) {
        match action {
            TextAction::Move(movement) => {
                let sel = super::movement(movement, self.selection, &self.layout, false);
                self.external_selection_change = Some(sel);
                self.scroll_to_selection_end(false);
            }
            TextAction::MoveSelecting(movement) => {
                let sel = super::movement(movement, self.selection, &self.layout, true);
                self.external_selection_change = Some(sel);
                self.scroll_to_selection_end(false);
            }
            ...
            TextAction::SelectWord => {
                if self.selection.is_caret() {
                    let range =
                        super::movement::word_range_for_pos(buffer.as_str(), self.selection.active);
            ...
            TextAction::Delete(movement) if self.selection.is_caret() => {
                if movement == Movement::Grapheme(druid_shell::text::Direction::Upstream) {
                    ...
                    let to_delete = super::movement(movement, self.selection, &self.layout, true);
    ...
    fn backspace(&mut self, buffer: &mut T) {
        let to_del = if self.selection.is_caret() {
            let del_start = super::offset_for_delete_backwards(&self.selection, buffer);
    ...
    fn word_for_pos(&self, pos: usize) -> Range<usize> {
        ...
        let mut range = super::movement::word_range_for_pos(text, rel_pos);
...
impl<T: TextStorage + EditableText> InputHandler for EditSessionHandle<T> {
    ...
    fn hit_test_point(&self, point: Point) -> druid::piet::HitTestPoint {
        self.inner
            .borrow()
            .layout
            .layout()
