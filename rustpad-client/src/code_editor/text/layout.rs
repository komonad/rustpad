...
use druid::{Env, FontDescriptor, KeyOrValue, PaintCtx, RenderContext, UpdateCtx};
...
impl<T> TextLayout<T> {
    ...
    pub fn new() -> Self {
        TextLayout {
            ...
            text_color: druid::theme::TEXT_COLOR.into(),
...
impl<T: TextStorage> TextLayout<T> {
    ...
    pub fn set_text(&mut self, text: T) {
        if self.text.is_none() || !self.text.as_ref().unwrap().same(&text) {
            self.text_is_rtl = druid::piet::util::first_strong_rtl(text.as_str());
            self.text = Some(text);
            self.layout = None;
        }
    }
